	...
	if (bio) {
		if (io->io_wbc->sync_mode == WB_SYNC_ALL)
			io->io_bio->bi_opf |= REQ_SYNC;
                //scorw start
                //send flush flags for friend file's blocks only
                //printk("ext4_io_submit: io->io_wbc: %p\n", io->io_wbc);
                //printk("ext4_io_submit: io->io_wbc->is_attached_to_friend_file: %d\n", io->io_wbc->is_attached_to_friend_file);
                //This if condition will be true only when get_child_inode_num function pointer is set in corw_sparse.c
                if(io->io_wbc->is_attached_to_friend_file)
                {
                        //Dereferencing io->io_wbc->inode to find inode num throws null pointer dereference error. Why?
                        //printk("ext4_io_submit: io->io_wbc->inode: %p\n", io->io_wbc->inode);
                        //printk("ext4_io_submit: Submitting friend file's (%lu) bio with flush and fua flags\n", io->io_wbc->inode->i_ino);
                        //printk("ext4_io_submit: bio: %p\n", bio);
                        //printk("ext4_io_submit: bio->bi_opf: %u\n", bio->bi_opf);
                        bio->bi_opf = REQ_PREFLUSH | bio->bi_opf;
                        //bio->bi_opf = REQ_FUA | bio->bi_opf;
                }

                if(io->io_wbc->is_attached_to_parent_file)
                {
                        //Dereferencing io->io_wbc->inode to find inode num throws null pointer dereference error. Why?
                        //printk("ext4_io_submit: io->io_wbc->inode: %p\n", io->io_wbc->inode);
                        //printk("ext4_io_submit: Submitting parent file's (%lu) bio with flush and fua flags\n", io->io_wbc->inode->i_ino);
                        //printk("ext4_io_submit: bio: %p\n", bio);
                        //printk("ext4_io_submit: bio->bi_opf: %u\n", bio->bi_opf);
                        bio->bi_opf = REQ_PREFLUSH | bio->bi_opf;
                        //bio->bi_opf = REQ_FUA | bio->bi_opf;
