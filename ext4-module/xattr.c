	       ...
	       const void *value, size_t value_len, int flags)
{
	...
	int j = 0;
	int buf_size = 0;
	unsigned long init_version = 1;	//version numbering starts from 1
	struct inode *p_inode = 0;	//parent inode val
	struct inode *f_inode = 0;	//friend inode val
	struct helper_inodes *h_inodes = 0;
	unsigned scorw_child_inode_num = 0;	
	unsigned blocks_count;
	extern char *copy_size;
	extern char *blocks_to_copy;
	extern char *scorw_parent;
	extern char *scorw_friend;
	...
	//printk("Inside ext4_xattr_set\n");
	//
	//print info about inodes in scorw inodes list
        //scorw_print_inode_list();
	//printk("ext4_xattr_set: name_index: %d\n", name_index);
	//printk("ext4_xattr_set: name: %s\n", name);
	
	/*	
	if(((unsigned*)value) != NULL)
	{
		h_inodes = (struct helper_inodes*)value;
		printk("ext4_xattr_set: values: %lu, %lu\n", h_inodes->p_inode_num, h_inodes->f_inode_num);
	}	
	else
	{
		printk("ext4_xattr_set: value: NULL\n");
	}
	*/
	...
	//printk("ext4_xattr_set: sizeof(unsigned long): %d\n", sizeof(unsigned long));
	//printk("ext4_xattr_set: value_len: %d\n", value_len);
	//printk("ext4_xattr_set: flags: %d\n", flags);
	//printk("ext4_xattr_set: inode->i_size: %lu\n", c_inode->i_size);


	
	/*
	//child file should be empty
	if((strcmp(name, scorw_parent) == 0) && (c_inode->i_size != 0) && (((unsigned*)value) != NULL))
	{
		printk("SCORW_ERR_NON_EMPTY_FILE error. Trying to set parent for non empty file.\n");
		return SCORW_ERR_NON_EMPTY_FILE;
	}
	*/

	//setting parent attribute of a child inode
	if((strcmp(name, scorw_parent) == 0) && (((unsigned*)value) != NULL))
	{
	        h_inodes = (struct helper_inodes *)value;

		//printk("parent's inode number: %lu\n", h_inodes->p_inode_num);
		...
		//This lock makes sure that writeback, sync and creation of child happens atomically
                mutex_lock(&(p_inode->i_vfs_inode_writeback_sync_lock));
		
		//This lock makes sure that open(par),close(par) and creation of par scorw inodes inside special_open() happens atomically
		//We are acquiring this lock here instead of inside special_open() to maintain lock ordering
                mutex_lock(&(p_inode->i_vfs_inode_open_close_lock));

		//We want to perform the creation of extended attributes and call to 
		//special_open() (function that attaches newly created child
		//to par if it exists) atomically.
		//So, use this lock.
		//Observe that we need to acquire par vfs's inode lock
		mutex_lock(&(p_inode->i_vfs_inode_lock));

		//====================================
		//Setting parents extended attributes
		//====================================
		//printk("Setting parents extended attributes\n");
		

		//donot create parent child relationship if parent file is empty
		if (p_inode->i_size != 0) 
		{

			//find empty attribute of parent where child inode number 
			//can be inserted
			for(i = 0; i < SCORW_MAX_CHILDS; i++)
			{
				memset(scorw_child_name, '\0', CHILD_NAME_LEN);
				sprintf(scorw_child_name, "%s%d", scorw_child, i);

				buf_size = ext4_xattr_get(p_inode, 1, scorw_child_name, &scorw_child_inode_num , sizeof(unsigned long));
				if((buf_size < 0) || (scorw_child_inode_num ==0))
				{
					break;
				}
			}
			//error; parent already has max children allowed.
			//Thus, can't add new child.
			if(i == SCORW_MAX_CHILDS)
			{
				printk("i == SCORW_MAX_CHILDS. Returning.");
				mutex_unlock(&(p_inode->i_vfs_inode_lock));
				mutex_unlock(&(p_inode->i_vfs_inode_open_close_lock));
				mutex_unlock(&(p_inode->i_vfs_inode_writeback_sync_lock));
				iput(p_inode);
				iput(f_inode);
				return SCORW_ERR_MAX_CHILDS_ALREADY_SET;
			}
			//empty attribute slot found. 
			//insert child inode in this attribute of parent
			if((i < SCORW_MAX_CHILDS) && ((buf_size < 0) || (scorw_child_inode_num ==0))) 
			{
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				//Saving info about child inode in par inode
				////////////////////////////////////////////////////////////////////////////////////////////////////////
				ext4_xattr_set(p_inode, 1, scorw_child_name, &(c_inode->i_ino), sizeof(unsigned long), 0);
			}

			//change dest file size
			//need to lock w.r.t. inode size? 
			//
			c_inode->i_size = p_inode->i_size;
			ext4_update_i_disksize(c_inode, p_inode->i_size);
			mark_inode_dirty(c_inode);
			//printk("%s(): p_inode->i_size: %ld, c_inode->i_size: %ld\n", __func__, p_inode->i_size, c_inode->i_size);

			////////////////////////////////////////////////////////////////////////////////////////////////////////
			//Saving info about parents size in terms of bytes and count of blocks yet to be copied in child inode
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			ext4_xattr_set(c_inode, 1, copy_size, &(c_inode->i_size), sizeof(long long), 0);

			blocks_count = (((c_inode->i_size - 1) >> PAGE_SHIFT) + 1);
			ext4_xattr_set(c_inode, 1, blocks_to_copy, &(blocks_count), sizeof(unsigned), 0);

			////////////////////////////////////////////////////////////////////////////////////////////////////////
			//Saving info about friend inode in child inode
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			ext4_xattr_set(c_inode, 1, scorw_friend, &(h_inodes->f_inode_num), sizeof(unsigned long), 0);

			////////////////////////////////////////////////////////////////////////////////////////////////////////
			//Saving info about child inode in friend inode
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			ext4_xattr_set(f_inode, 1, scorw_child_frnd, &(c_inode->i_ino), sizeof(unsigned long), 0);

			////////////////////////////////////////////////////////////////////////////////////////////////////////
			//Initialise version information in child and frnd inode
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			////////////////////////////////////////////////////////////////////////////////////////////////////////
			ext4_xattr_set(c_inode, 1, num_ranges, &(h_inodes->num_ranges), sizeof(int), 0);
			for(j = 0; j < h_inodes->num_ranges; j++)
			{
		                #ifdef USE_OLD_RANGE	
				//range start
				memset(scorw_range_name, '\0', CHILD_RANGE_LEN);
				sprintf(scorw_range_name, "%s%d", scorw_range_start, j);
				ext4_xattr_set(c_inode, 1, scorw_range_name , &(h_inodes->range[j].start), sizeof(unsigned), 0);

				//range end
				memset(scorw_range_name, '\0', CHILD_RANGE_LEN);
				sprintf(scorw_range_name, "%s%d", scorw_range_end, j);
				ext4_xattr_set(c_inode, 1, scorw_range_name , &(h_inodes->range[j].end), sizeof(unsigned), 0);
                                
                                #else  //Generic sharing scheme
				struct child_range_xattr crx;  //pack the attribute into this structure
				memset(scorw_range_name, '\0', CHILD_RANGE_LEN);
				sprintf(scorw_range_name, "%s%d", scorw_range_start, j);
		...
		{
			printk("Parent's file size is 0\n");
			mutex_unlock(&(p_inode->i_vfs_inode_lock));
			mutex_unlock(&(p_inode->i_vfs_inode_open_close_lock));
			mutex_unlock(&(p_inode->i_vfs_inode_writeback_sync_lock));
			iput(p_inode);
			iput(f_inode);
			return SCORW_PARENT_ERROR;
		}
	}
...
out:
	//printk("ext4_xattr_set: name: %s, return value: %d\n", name, error);
	//setting parent attribute of a child inode
	if((strcmp(name, scorw_parent) == 0) && (((unsigned*)value) != NULL))
	{
		//donot create parent child relationship if parent file is empty
		if (p_inode->i_size != 0) 
		{
			//special open should be called after all the attributes have been set and frnd file has been initialized
			if(!error)
			{
				scorw_init_friend_file(f_inode, c_inode->i_size);
				special_open(p_inode, c_inode, i);
			}
		}
		mutex_unlock(&(p_inode->i_vfs_inode_lock));
		mutex_unlock(&(p_inode->i_vfs_inode_open_close_lock));
