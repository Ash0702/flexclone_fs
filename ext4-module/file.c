...
ext4_file_write_iter(struct kiocb *iocb, struct iov_iter *from)
{
	...
	if (IS_DAX(inode)){
		...
		scorw_set_transaction(inode , UNSET_TRANSACTION);
		return ret;
	}
#endif
	if (iocb->ki_flags & IOCB_DIRECT){
...
//	}
		...
		if(IS_ERR_VALUE(p_inode)) {
			printk("[WIERD] :: stored parent_attr_val for c_ino_num=%d is invalid\n" , inode->i_ino);
		}
		...
		printk("Returned to file.c for i_ino=%d\n" , inode->i_ino);
...
again:
		...
		struct folio* folio;
		status = a_ops->write_begin(file, mapping, pos, bytes, &folio, &fsdata); //Rajan folio patch
		page= &(folio->page);
		...
		//copied = iov_iter_copy_from_user_atomic(page, i, offset, bytes); (patched Rajan)
