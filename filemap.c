...
#include "swap.h"
//scorw start
#include "../fs/ext4-module/ext4.h"
#include "../fs/ext4-module/corw_sparse.h"
#include <linux/pagevec.h>
//scorw end


//scorw start
wait_queue_head_t sync_child_wait_queue;
//init_waitqueue_head (&wq);                    //This wait queue is initialized in ext4-module
EXPORT_SYMBOL(sync_child_wait_queue);
...
extern unsigned long (*get_child_i_attr_val)(struct inode *inode, int child_i);
extern unsigned long (*get_friend_attr_val)(struct inode *inode);
extern int (*is_block_copied)(struct inode *inode, unsigned blk_num);
extern struct page_copy *(*find_page_copy)(unsigned block_num, unsigned long par_ino_num, int child_index);
int scorw_fsync_journal(struct inode *inode, bool datasync, bool *needs_barrier);
int scorw_inode_write_and_wait_range(struct inode *inode, loff_t lstart, loff_t lend);
...
//scorw start
//This is copy of ext4_fsync_journal
int scorw_fsync_journal(struct inode *inode, bool datasync, bool *needs_barrier)
{
...
EXPORT_SYMBOL(scorw_fsync_journal);
			   ...
			   struct writeback_control *wbc)
{
	...
	//Rajan
	int ret;
	loff_t start = wbc->range_start;
	loff_t end = wbc->range_end;
	int sync_mode= wbc->sync_mode;

	struct inode *inode = mapping->host;	//parent inode
	unsigned start_block_num = (start >> PAGE_SHIFT);
	unsigned end_block_num = (end >> PAGE_SHIFT);
        unsigned long child_inode_num = 0;
        struct inode *child_inode = 0;
        unsigned long friend_inode_num = 0;
        struct inode *friend_inode = 0;
	struct super_block *sb = 0;
        //struct pagevec pvec; Rajan
		struct folio_batch fbatch;
        unsigned long nr_pages = 0;
        pgoff_t index = 0;
        unsigned long i = 0;
        unsigned long j = 0;
        struct page *child_page = 0;
	...
	*/
	...
	if(is_ext4_module_sb(inode->i_sb) && (inode->i_ino != 0) && is_par_inode && (is_par_inode(inode, 1)))
	{
		...
		//
		//Note: We use inode->i_vfs_inode_open_close_lock instead of inode->i_vfs_inode_lock (both disallow creation of children) because 
		//	usage of inode->i_vfs_inode_lock can result deadlock.
		//	Eg: 	i_vfs_inode_lock is acquired here and then wait_event() is called while aquiring this lock.
		//	     	Meanwhile page copy thread also has to aquire i_vfs_inode_lock to process
		//	     	page copy structs. But it can't acquire.
		//	     	This thread has called wait_event() and it can't wakeup until particular page copy it is waiting
		//		for is not processed. But page copy thread can't process that page copy struct because
		//		it can't acquire the i_vfs_inode_lock.
			...
			if(nr_pages)
			{
				...
				for (i = 0; i < nr_pages; i++)
				{
					...
					struct writeback_control child_wbc = {
						...
						.range_cyclic           = 0,
						.nr_to_write 		= 1,
						.range_start 		= (page->index << PAGE_SHIFT),  //in bytes
						.range_end 		= (page->index << PAGE_SHIFT), //in bytes. range_end is inclusive
					};
					
					//MAHA_AARSH_start (since no child needs a writeback in our case)
					//process each child
					/*for(j = 0; j < SCORW_MAX_CHILDS; j++)
					{
						child_inode_num = get_child_i_attr_val(inode, j);
						if(child_inode_num == 0)
						{
							continue;
						}

						//Don't forget to do iput()
						child_inode = ext4_iget(sb, child_inode_num, EXT4_IGET_NORMAL);
						if(IS_ERR_OR_NULL(child_inode))
						{
							continue;
						}
						...
						//Todo: Modify unlink path to achieve this.
						friend_inode_num = get_friend_attr_val(child_inode);
						friend_inode = ext4_iget(sb, friend_inode_num, EXT4_IGET_NORMAL);
						if(IS_ERR_OR_NULL(friend_inode))
						{
							iput(child_inode);
							continue;
						}
						...
						//the range of child and the append operations performed on par
						//
						//2) Call to find_page_copy should be made first and then the call to is_block_copied
						//
						//Todo: There is an edge case here which needs to be handled.
						//      write(blk 0 of par)
						//      create(child, say, c2)
						//      write(blk 0 of par)
						//      write on parent happens after the below condition returned true
						//      for child c2.
						//      Now, parent's contents get written to disk while child's contents
						//      may still be in page copy or in child's page cache
						//
						//Update:
						//      This edge case is handled by acquiring par vfs lock (i_vfs_inode_lock).
						//      This same lock is required for the creation of new child
						//
						...
						//This implies child page has already been written to disk.
						...
						//child page exists in page cache. It should be processed only
						//if it is dirty
						//
						//If it is not dirty, it implies, it is under writeback or has
		...
		//	Child blk 'b' is written. It's extent is updated.
		//	Now, updated metadata of child (updated extent tree in this case) should also be written
		//	before the parent block is written.
		//	Otherwise following issue can arise:
		//		child blk written, parent block written, however, transaction containing the updated
		//		extent tree of child is not committed yet and the system crashes.
		//		On recovery, child blk is lost because extent tree was not updated
		//		on top of that contents of parent have been modified.
		/* MAHA_AARSH_start for(j = 0; j < SCORW_MAX_CHILDS; j++)
		{
			child_inode_num = get_child_i_attr_val(inode, j);
			if(child_inode_num == 0)
			{
...
EXPORT_SYMBOL(scorw_inode_write_and_wait_range);
...
vm_fault_t filemap_fault(struct vm_fault *vmf)
{
	...
	if (likely(!IS_ERR(folio))) {
		...
		if (!(vmf->flags & FAULT_FLAG_TRIED))
			fpin = do_async_mmap_readahead(vmf, folio); //scorw start end//
		...
		ret = VM_FAULT_MAJOR;
		fpin = do_sync_mmap_readahead(vmf); //scorw start end//
retry_find:
		/*
